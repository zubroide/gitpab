...
@section('tableTbody')
    @forelse ($itemsList->items() as $key => $item)
        <tr>
            ...
            @endif
            <td class="col-md-2 table-action-column">
                @if (!$item->deleted_at)
                    <a href="{{ route($destroyRoute, [$item->id]) }}"
                       class="btn btn-xs text-danger crud_destroy_btn_js pull-right"
                ...
                @else
                    <a href="{{ route($restoreRoute, [$item->id]) }}"
                       class="btn btn-xs crud_restore_btn_js pull-right"
                       data-token="{{ csrf_token() }}"
                ...
                @if (!$item->deleted_at)
                    <a href="{{ route($editRoute, [$item->id]) }}"
                       class="btn btn-xs text-success pull-right"
                       data-token="{{ csrf_token() }}">
